<script lang="ts">
	import type { HTMLAttributes } from 'svelte/elements';

	export type iconType =
		| 'fullscreen'
		| 'marker'
		| 'globePlus'
		| 'imgError'
		| 'signOut'
		| 'reset'
		| 'addImage'
		| 'selectImages'
		| 'leftArrow'
		| 'rightArrow'
		| 'delete'
		| 'rename'
		| 'x'
		| 'selectAll'
		| 'unselectAll'
		| 'profile'
		| 'spinner'
		| 'calendar'
		| 'settings'
		| 'settingsLarge';

	type Props = {
		type: iconType;
		color?: string;
		fill?: string;
		stroke?: string;
		scale?: number;
		hoverScale?: boolean;
		disabled?: boolean;
	} & HTMLAttributes<HTMLDivElement>;

	let { type, color = 'white', scale = 1, hoverScale = false, disabled, ...rest }: Props = $props();

	const classMap = new Map<string, string>();
	classMap.set('spinner', 'h-4 w-4 animate-spin');

	// svelte-ignore state_referenced_locally
	if (disabled) {
		color = 'grey';
		hoverScale = false;
	}
</script>

<div class="size-fit {hoverScale ? 'hover:scale-[120%]' : ''}" {...rest}>
	{#if type != 'imgError'}
		<svg
			width={24 * scale}
			height={24 * scale}
			viewBox="0 0 24 24"
			fill={color}
			stroke={color}
			xmlns="http://www.w3.org/2000/svg"
			class={classMap.get(type)}
		>
			{#if type === 'fullscreen'}
				<path
					d="M9.00002 3.99998H4.00004L4 9M20 8.99999V4L15 3.99997M15 20H20L20 15M4 15L4 20L9.00002 20"
					stroke-width="2.3"
					stroke-linecap="round"
					stroke-linejoin="round"
					fill="none"
				></path>
			{/if}
			{#if type === 'marker'}
				<g fill="none">
					<path
						d="M19 10C19 13.9765 12 21 12 21C12 21 5 13.9765 5 10C5 6.02355 8.13401 3 12 3C15.866 3 19 6.02355 19 10Z"
						stroke-linecap="round"
						stroke-linejoin="round"
						stroke-width="2"
					></path><circle
						cx="12"
						cy="10"
						r="3"
						stroke-linecap="round"
						stroke-linejoin="round"
						stroke-width="2"
					></circle>
				</g>
			{/if}
			{#if type === 'globePlus'}
				<g stroke="none">
					<path
						fill-rule="evenodd"
						clip-rule="evenodd"
						d="M12 2.25C6.615 2.25 2.25 6.615 2.25 12C2.25 17.385 6.615 21.75 12 21.75C17.385 21.75 21.75 17.385 21.75 12C21.75 6.615 17.385 2.25 12 2.25ZM8.547 4.505C7.23052 5.11184 6.09718 6.05483 5.2611 7.23903C4.42502 8.42323 3.91573 9.80683 3.7845 11.2505C3.65327 12.6941 3.90474 14.1469 4.5136 15.4624C5.12245 16.778 6.06718 17.9099 7.25266 18.7441C8.43814 19.5784 9.82251 20.0856 11.2664 20.2146C12.7102 20.3436 14.1626 20.0899 15.4772 19.479C16.7918 18.8682 17.9222 17.9217 18.7547 16.735C19.5871 15.5482 20.0922 14.163 20.219 12.719L19.759 12.259C19.5877 12.0876 19.4452 11.8898 19.337 11.673L18.257 9.513C18.2276 9.45438 18.1847 9.40358 18.1318 9.36478C18.079 9.32598 18.0176 9.3003 17.9529 9.28986C17.8881 9.27941 17.8218 9.28449 17.7594 9.30468C17.697 9.32487 17.6403 9.3596 17.594 9.406C17.4897 9.51022 17.3594 9.58465 17.2166 9.62157C17.0738 9.6585 16.9238 9.65657 16.782 9.616L15.509 9.253C15.3053 9.19574 15.0879 9.21291 14.8958 9.30143C14.7036 9.38996 14.5492 9.54404 14.4604 9.73605C14.3716 9.92807 14.354 10.1455 14.4109 10.3492C14.4679 10.553 14.5955 10.7298 14.771 10.848L15.358 11.238C15.948 11.633 16.032 12.468 15.53 12.97L15.33 13.17C15.119 13.382 15 13.668 15 13.966V14.376C15 14.785 14.89 15.185 14.68 15.534L13.365 17.725C13.1775 18.0377 12.9123 18.2964 12.5951 18.476C12.2779 18.6557 11.9195 18.7501 11.555 18.75C11.2752 18.75 11.0069 18.6388 10.809 18.441C10.6112 18.2431 10.5 17.9748 10.5 17.695V16.523C10.5 15.603 9.94 14.776 9.086 14.434L8.432 14.173C7.95525 13.9824 7.55803 13.6345 7.30628 13.187C7.05453 12.7395 6.96339 12.2194 7.048 11.713L7.055 11.671C7.10148 11.3928 7.19986 11.1258 7.345 10.884L7.435 10.734C7.67382 10.3363 8.02972 10.0222 8.45398 9.83455C8.87824 9.64695 9.35011 9.59506 9.805 9.686L10.983 9.922C11.2602 9.97729 11.5481 9.9262 11.7893 9.77889C12.0306 9.63158 12.2075 9.39885 12.285 9.127L12.493 8.397C12.5651 8.14481 12.5467 7.87536 12.4412 7.63524C12.3356 7.39513 12.1495 7.19944 11.915 7.082L11.25 6.75L11.159 6.841C10.9501 7.04993 10.702 7.21566 10.429 7.32874C10.1561 7.44181 9.86348 7.5 9.568 7.5H9.388C9.139 7.5 8.901 7.6 8.726 7.774C8.56562 7.93584 8.35145 8.03324 8.12407 8.04776C7.89669 8.06229 7.67187 7.99293 7.4922 7.85281C7.31253 7.7127 7.19048 7.51156 7.14916 7.28749C7.10784 7.06343 7.15012 6.83198 7.268 6.637L8.547 4.505Z"
					/>
					<rect x="19" y="2" width="5" height="2" />
					<rect x="22.5" y="0.5" width="5" height="2" transform="rotate(90 22.5 0.5)" />
				</g>
			{/if}
			{#if type === 'addImage'}
				<path
					stroke="none"
					d="M23 4v2h-3v3h-2V6h-3V4h3V1h2v3h3zm-8.5 7c.828 0 1.5-.672 1.5-1.5S15.328 8 14.5 8 13 8.672 13 9.5s.672 1.5 1.5 1.5zm3.5 3.234l-.513-.57c-.794-.885-2.18-.885-2.976 0l-.655.73L9 9l-3 3.333V6h7V4H6c-1.105 0-2 .895-2 2v12c0 1.105.895 2 2 2h12c1.105 0 2-.895 2-2v-7h-2v3.234z"
				></path>
			{/if}
			{#if type === 'selectImages'}
				<g stroke="none">
					<path
						fill-rule="evenodd"
						d="M13,8 L13,6 L19,6 C20.1045695,6 21,6.8954305 21,8 L21,19 C21,20.1045695 20.1045695,21 19,21 L8,21 C6.8954305,21 6,20.1045695 6,19 L6,13 L8,13 L8,19 L19,19 L19,8 L13,8 Z M6,6 L6,3 L8,3 L8,6 L11,6 L11,8 L8,8 L8,11 L6,11 L6,8 L3,8 L3,6 L6,6 Z"
					></path>
				</g>
			{/if}
			{#if type === 'signOut'}
				<g stroke="none">
					<path
						d="M13,15a1,1,0,0,0-1,1v4H4V4h8V8a1,1,0,0,0,2,0V4a2,2,0,0,0-2-2H4A2,2,0,0,0,2,4V20a2,2,0,0,0,2,2h8a2,2,0,0,0,2-2V16A1,1,0,0,0,13,15Z"
					></path><path
						d="M21.92,11.62a1.15,1.15,0,0,0-.21-.33h0l-4-4a1,1,0,1,0-1.42,1.42L18.59,11H8a1,1,0,0,0,0,2H18.59l-2.3,2.29a1,1,0,0,0,0,1.42,1,1,0,0,0,1.42,0l4-4h0a1.15,1.15,0,0,0,.21-.33.94.94,0,0,0,0-.76Z"
					></path>
				</g>
			{/if}
			{#if type === 'reset'}
				<path
					fill="none"
					stroke-width="1.5"
					d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"
				/><path stroke-width="1.5" d="M3 3v5h5" />
			{/if}
			{#if type === 'leftArrow'}
				<path
					fill="none"
					d="M19 12H5M12 19l-7-7 7-7"
					stroke-width="2"
					stroke-linecap="round"
					stroke-linejoin="round"
				/>
			{/if}
			{#if type === 'rightArrow'}
				<path
					fill="none"
					d="M5 12h14M12 5l7 7-7 7"
					stroke-width="2"
					stroke-linecap="round"
					stroke-linejoin="round"
				/>
			{/if}
			{#if type === 'delete'}
				<g fill="none">
					<path d="M10 12V17" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
					></path>
					<path d="M14 12V17" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
					></path>
					<path d="M4 7H20" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
					<path
						d="M6 10V18C6 19.6569 7.34315 21 9 21H15C16.6569 21 18 19.6569 18 18V10"
						stroke-width="2"
						stroke-linecap="round"
						stroke-linejoin="round"
					></path>
					<path
						d="M9 5C9 3.89543 9.89543 3 11 3H13C14.1046 3 15 3.89543 15 5V7H9V5Z"
						stroke-width="2"
						stroke-linecap="round"
						stroke-linejoin="round"
					></path>
				</g>
			{/if}
			{#if type === 'rename'}
				<g stroke="none">
					<path
						fill-rule="evenodd"
						clip-rule="evenodd"
						d="M10 4H8V6H5C3.34315 6 2 7.34315 2 9V15C2 16.6569 3.34315 18 5 18H8V20H10V4ZM8 8V16H5C4.44772 16 4 15.5523 4 15V9C4 8.44772 4.44772 8 5 8H8Z"
					></path>
					<path
						d="M19 16H12V18H19C20.6569 18 22 16.6569 22 15V9C22 7.34315 20.6569 6 19 6H12V8H19C19.5523 8 20 8.44771 20 9V15C20 15.5523 19.5523 16 19 16Z"
					></path>
				</g>
			{/if}
			{#if type === 'x'}
				<line x1="4" y1="4" x2="20" y2="20" stroke-width="2" stroke-linecap="round" />
				<line x1="20" y1="4" x2="4" y2="20" stroke-width="2" stroke-linecap="round" />
			{/if}
			{#if type === 'selectAll'}
				<g id="ic_fluent_select_all_24_regular" stroke="none" fill-rule="nonzero">
					<path
						d="M20.4961766,5.62668182 C21.3720675,5.93447702 22,6.76890777 22,7.75 L22,17.75 C22,20.0972102 20.0972102,22 17.75,22 L7.75,22 C6.76890777,22 5.93447702,21.3720675 5.62668182,20.4961766 L7.72396188,20.4995565 L17.75,20.5 C19.2687831,20.5 20.5,19.2687831 20.5,17.75 L20.5,7.75 L20.4960194,7.69901943 L20.4961766,5.62668182 Z M17.246813,2 C18.4894537,2 19.496813,3.00735931 19.496813,4.25 L19.496813,17.246813 C19.496813,18.4894537 18.4894537,19.496813 17.246813,19.496813 L4.25,19.496813 C3.00735931,19.496813 2,18.4894537 2,17.246813 L2,4.25 C2,3.00735931 3.00735931,2 4.25,2 L17.246813,2 Z M17.246813,3.5 L4.25,3.5 C3.83578644,3.5 3.5,3.83578644 3.5,4.25 L3.5,17.246813 C3.5,17.6610266 3.83578644,17.996813 4.25,17.996813 L17.246813,17.996813 C17.6610266,17.996813 17.996813,17.6610266 17.996813,17.246813 L17.996813,4.25 C17.996813,3.83578644 17.6610266,3.5 17.246813,3.5 Z M9.58114564,11.3581942 L13.4696699,7.46966991 C13.7625631,7.1767767 14.2374369,7.1767767 14.5303301,7.46966991 C14.7965966,7.73593648 14.8208027,8.15260016 14.6029482,8.44621165 L14.5303301,8.53033009 L10.0303301,13.0303301 C9.73449239,13.3261678 9.26134027,13.3189471 8.97360394,13.0344464 L8.9,12.95 L7.4,10.95 C7.15147186,10.6186292 7.21862915,10.1485281 7.55,9.9 C7.85124623,9.67406533 8.26714548,9.7090277 8.52699676,9.96621555 L8.6,10.05 L9.58114564,11.3581942 L13.4696699,7.46966991 L9.58114564,11.3581942 Z"
					>
					</path>
				</g>
			{/if}
			{#if type === 'unselectAll'}
				<g id="ic_fluent_select_all_off_24_regular" stroke="none" fill-rule="nonzero">
					<path
						d="M20.4961766,5.62668182 C21.3720675,5.93447702 22,6.76890777 22,7.75 L22,17.75 C22,20.0972102 20.0972102,22 17.75,22 L7.75,22 C6.76890777,22 5.93447702,21.3720675 5.62668182,20.4961766 L7.72396188,20.4995565 L17.75,20.5 C19.2687831,20.5 20.5,19.2687831 20.5,17.75 L20.5,7.75 L20.4960194,7.69901943 L20.4961766,5.62668182 Z M17.246813,2 C18.4894537,2 19.496813,3.00735931 19.496813,4.25 L19.496813,17.246813 C19.496813,18.4894537 18.4894537,19.496813 17.246813,19.496813 L4.25,19.496813 C3.00735931,19.496813 2,18.4894537 2,17.246813 L2,4.25 C2,3.00735931 3.00735931,2 4.25,2 L17.246813,2 Z M17.246813,3.5 L4.25,3.5 C3.83578644,3.5 3.5,3.83578644 3.5,4.25 L3.5,17.246813 C3.5,17.6610266 3.83578644,17.996813 4.25,17.996813 L17.246813,17.996813 C17.6610266,17.996813 17.996813,17.6610266 17.996813,17.246813 L17.996813,4.25 C17.996813,3.83578644 17.6610266,3.5 17.246813,3.5 Z"
					>
					</path>
				</g>
			{/if}
			{#if type === 'profile'}
				<g stroke="none">
					<path
						d="M22 12C22 6.49 17.51 2 12 2C6.49 2 2 6.49 2 12C2 14.9 3.25 17.51 5.23 19.34C5.23 19.35 5.23 19.35 5.22 19.36C5.32 19.46 5.44 19.54 5.54 19.63C5.6 19.68 5.65 19.73 5.71 19.77C5.89 19.92 6.09 20.06 6.28 20.2C6.35 20.25 6.41 20.29 6.48 20.34C6.67 20.47 6.87 20.59 7.08 20.7C7.15 20.74 7.23 20.79 7.3 20.83C7.5 20.94 7.71 21.04 7.93 21.13C8.01 21.17 8.09 21.21 8.17 21.24C8.39 21.33 8.61 21.41 8.83 21.48C8.91 21.51 8.99 21.54 9.07 21.56C9.31 21.63 9.55 21.69 9.79 21.75C9.86 21.77 9.93 21.79 10.01 21.8C10.29 21.86 10.57 21.9 10.86 21.93C10.9 21.93 10.94 21.94 10.98 21.95C11.32 21.98 11.66 22 12 22C12.34 22 12.68 21.98 13.01 21.95C13.05 21.95 13.09 21.94 13.13 21.93C13.42 21.9 13.7 21.86 13.98 21.8C14.05 21.79 14.12 21.76 14.2 21.75C14.44 21.69 14.69 21.64 14.92 21.56C15 21.53 15.08 21.5 15.16 21.48C15.38 21.4 15.61 21.33 15.82 21.24C15.9 21.21 15.98 21.17 16.06 21.13C16.27 21.04 16.48 20.94 16.69 20.83C16.77 20.79 16.84 20.74 16.91 20.7C17.11 20.58 17.31 20.47 17.51 20.34C17.58 20.3 17.64 20.25 17.71 20.2C17.91 20.06 18.1 19.92 18.28 19.77C18.34 19.72 18.39 19.67 18.45 19.63C18.56 19.54 18.67 19.45 18.77 19.36C18.77 19.35 18.77 19.35 18.76 19.34C20.75 17.51 22 14.9 22 12ZM16.94 16.97C14.23 15.15 9.79 15.15 7.06 16.97C6.62 17.26 6.26 17.6 5.96 17.97C4.44 16.43 3.5 14.32 3.5 12C3.5 7.31 7.31 3.5 12 3.5C16.69 3.5 20.5 7.31 20.5 12C20.5 14.32 19.56 16.43 18.04 17.97C17.75 17.6 17.38 17.26 16.94 16.97Z"
					></path>
					<path
						d="M12 6.92969C9.93 6.92969 8.25 8.60969 8.25 10.6797C8.25 12.7097 9.84 14.3597 11.95 14.4197C11.98 14.4197 12.02 14.4197 12.04 14.4197C12.06 14.4197 12.09 14.4197 12.11 14.4197C12.12 14.4197 12.13 14.4197 12.13 14.4197C14.15 14.3497 15.74 12.7097 15.75 10.6797C15.75 8.60969 14.07 6.92969 12 6.92969Z"
					></path>
				</g>
			{/if}
			{#if type === 'spinner'}
				<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" />
				<path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4a4 4 0 00-4 4H4z" />
			{/if}
			{#if type === 'calendar'}
				<path
					d="M6,22H18a3,3,0,0,0,3-3V7a2,2,0,0,0-2-2H17V3a1,1,0,0,0-2,0V5H9V3A1,1,0,0,0,7,3V5H5A2,2,0,0,0,3,7V19A3,3,0,0,0,6,22ZM5,12.5a.5.5,0,0,1,.5-.5h13a.5.5,0,0,1,.5.5V19a1,1,0,0,1-1,1H6a1,1,0,0,1-1-1Z"
				/>
			{/if}
			{#if type === 'settings'}
				<path
					fill-rule="evenodd"
					d="M9.995 21.8c.655.13 1.33.2 2.005.2v-.005c.67 0 1.35-.065 2.005-.2c.58-.115.995-.63.995-1.225v-1.155a7.8 7.8 0 0 0 1.925-1.11l1 .575c.515.295 1.17.195 1.555-.245a10 10 0 0 0 2.01-3.48a1.25 1.25 0 0 0-.565-1.47l-1-.575a8.2 8.2 0 0 0 0-2.22l1-.58c.51-.295.75-.915.565-1.47a10 10 0 0 0-2.01-3.48c-.39-.44-1.04-.54-1.555-.245l-1 .575a8 8 0 0 0-1.92-1.11V3.425c0-.59-.42-1.11-.995-1.225c-1.31-.27-2.7-.27-4.015 0c-.58.115-.995.63-.995 1.225V4.58a7.8 7.8 0 0 0-1.925 1.11l-1-.575a1.25 1.25 0 0 0-1.555.245a10 10 0 0 0-2.01 3.48c-.185.555.05 1.175.565 1.47l1 .58a8.2 8.2 0 0 0 0 2.22l-1 .58c-.51.295-.75.915-.565 1.47a9.9 9.9 0 0 0 2.01 3.48c.39.44 1.04.54 1.555.245l1-.575c.59.46 1.235.83 1.925 1.11v1.155c0 .59.42 1.11.995 1.225m3.505-1.43a8.6 8.6 0 0 1-3 0v-2.02l-.52-.17a6.5 6.5 0 0 1-2.325-1.345l-.405-.365l-1.75 1.01A8.4 8.4 0 0 1 4 14.885l1.75-1.01l-.11-.53Q5.499 12.684 5.5 12c0-.455.05-.905.14-1.345l.11-.53L4 9.115A8.4 8.4 0 0 1 5.5 6.52l1.75 1.01l.405-.365A6.4 6.4 0 0 1 9.98 5.82l.52-.17V3.63a8.6 8.6 0 0 1 3 0v2.015l.52.17c.865.28 1.645.735 2.325 1.345l.405.365l1.75-1.01A8.4 8.4 0 0 1 20 9.11l-1.75 1.01l.11.53q.141.661.14 1.345c0 .455-.05.905-.14 1.345l-.11.53L20 14.88a8.4 8.4 0 0 1-1.5 2.6l-1.75-1.01l-.405.365a6.4 6.4 0 0 1-2.325 1.345l-.52.17zM8 12c0 2.205 1.795 4 4 4s4-1.795 4-4s-1.795-4-4-4s-4 1.795-4 4m1.5 0a2.5 2.5 0 0 1 5 0a2.5 2.5 0 0 1-5 0"
					clip-rule="evenodd"
				/>
			{/if}
			{#if type === 'settingsLarge'}
				<path
					d="M19.5 12c0-.23-.01-.45-.03-.68l1.86-1.41c.4-.3.51-.86.26-1.3l-1.87-3.23a.987.987 0 0 0-1.25-.42l-2.15.91c-.37-.26-.76-.49-1.17-.68l-.29-2.31c-.06-.5-.49-.88-.99-.88h-3.73c-.51 0-.94.38-1 .88l-.29 2.31c-.41.19-.8.42-1.17.68l-2.15-.91c-.46-.2-1-.02-1.25.42L2.41 8.62c-.25.44-.14.99.26 1.3l1.86 1.41a7.3 7.3 0 0 0 0 1.35l-1.86 1.41c-.4.3-.51.86-.26 1.3l1.87 3.23c.25.44.79.62 1.25.42l2.15-.91c.37.26.76.49 1.17.68l.29 2.31c.06.5.49.88.99.88h3.73c.5 0 .93-.38.99-.88l.29-2.31c.41-.19.8-.42 1.17-.68l2.15.91c.46.2 1 .02 1.25-.42l1.87-3.23c.25-.44.14-.99-.26-1.3l-1.86-1.41c.03-.23.04-.45.04-.68m-7.46 3.5c-1.93 0-3.5-1.57-3.5-3.5s1.57-3.5 3.5-3.5s3.5 1.57 3.5 3.5s-1.57 3.5-3.5 3.5"
				/>
			{/if}
		</svg>
	{/if}

	{#if type === 'imgError'}
		<svg
			width={36 * scale}
			height={36 * scale}
			viewBox="0 0 36 36"
			fill="none"
			xmlns="http://www.w3.org/2000/svg"
		>
			<g clip-path="url(#clip0_0_1)">
				<path
					d="M22 25L17.238 16.27L21 10L29 25H22ZM14 14L20 25H8L14 14ZM11.5 12C10.837 12 10.2011 11.7366 9.73223 11.2678C9.26339 10.7989 9 10.163 9 9.5C9 8.83696 9.26339 8.20107 9.73223 7.73223C10.2011 7.26339 10.837 7 11.5 7C12.163 7 12.7989 7.26339 13.2678 7.73223C13.7366 8.20107 14 8.83696 14 9.5C14 10.163 13.7366 10.7989 13.2678 11.2678C12.7989 11.7366 12.163 12 11.5 12Z"
					fill={color}
				/>
			</g>
			<path
				d="M18 33C26.2843 33 33 26.2843 33 18C33 9.71573 26.2843 3 18 3C9.71573 3 3 9.71573 3 18C3 26.2843 9.71573 33 18 33Z"
				stroke={color}
				stroke-width="2"
				stroke-linecap="round"
				stroke-linejoin="round"
			/>
			<path
				d="M7.5 28.5L28.5 7.5"
				stroke={color}
				stroke-width="2"
				stroke-linecap="round"
				stroke-linejoin="round"
			/>
			<defs>
				<clipPath id="clip0_0_1">
					<rect width={24 * scale} height={24 * scale} fill={color} transform="translate(6 4)" />
				</clipPath>
			</defs>
		</svg>
	{/if}
</div>
