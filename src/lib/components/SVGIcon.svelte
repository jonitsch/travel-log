<script lang="ts">
	interface Props {
		type: 'fullscreen' | 'marker' | 'globePlus' | 'imageError' | 'signOut' | 'reset';
		fill?: string;
		scale?: number;
		stroke?: string;
		hoverScale?: boolean;
...
{#if ['fullscreen', 'marker', 'globePlus'].includes(type)}
	<svg
...
{#if type === 'reset'}
	<svg
		...
		><path stroke-width="1.5" d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" /><path stroke-width="1.5" d="M3 3v5h5" /></svg
	>
{/if}
